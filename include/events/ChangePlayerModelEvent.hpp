...
namespace hs::event
{
    struct ChangePlayerModelEvent : public gengine::Event
    {
        ...
        ChangePlayerModelEvent(const std::string &modelPath)
            : modelPath(modelPath), isAnimated(false)
        {
        }

        ChangePlayerModelEvent(const std::string &modelPath, gengine::component::Transform3D transform)
            : modelPath(modelPath), isAnimated(false), transform(transform)
        {
        }
        ...
        ChangePlayerModelEvent(const std::string &modelPath, const std::string &animPath, float frameDuration,
            ...
            : modelPath(modelPath), isAnimated(true), animPath(animPath), frameDuration(frameDuration), transform(transform)
        {
        }

        ChangePlayerModelEvent(const std::string &modelPath, const std::string &animPath, float frameDuration,
                               gengine::Vect3 scale)
            : modelPath(modelPath), isAnimated(true), animPath(animPath), frameDuration(frameDuration)
        {
            transform = gengine::component::Transform3D({0, 0, 0}, scale);
        }
        ...
        ChangePlayerModelEvent(const std::string &modelPath, const std::string &animPath, float frameDuration,
            ...
            : modelPath(modelPath), isAnimated(true), animPath(animPath), frameDuration(frameDuration), playerUuid(playerUuid)
        {
            transform = gengine::component::Transform3D({0, 0, 0}, scale);
        }
    };
